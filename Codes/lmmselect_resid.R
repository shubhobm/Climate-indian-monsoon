...
random = Z %*% eta
r = y - fixed - random
r = sqrt(n/m) * (r - mean(r))/sqrt(1 - p/n)
eta = sqrt(nyr/myr) * (eta - mean(eta))/sqrt(1 - p/n)
...
for(i in 1:nboot){
	iind = sample(1:n, n, replace=T)
  iiNd = sample(1:nyr, nyr, replace=T)
  iyb = fixed + Z %*% eta[iiNd] + r[iind]
...
write.table(t, 'resid_m_ntothepoint3.txt')
