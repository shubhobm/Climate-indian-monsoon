...
rain_lo = rain[which(rain$PRCP < 644),]
rain_mid = rain[which(rain$PRCP>=644 & rain$PRCP<1244),]
rain_hi = rain[which(rain$PRCP>=1244),]


rain_lo_avg = aggregate(cbind(ELEVATION, PRCP, TMAX, TMIN, del_TT_Deg_Celsius, DMI, Nino34,
	Air_200_Temp, Air_600_Temp, u_wind_200, u_wind_600, u_wind_850,
	v_wind_200, v_wind_600, v_wind_850) ~ year + month + LATITUDE + LONGITUDE,
	data=rain_lo, FUN=mean)
rain_mid_avg = aggregate(cbind(ELEVATION, PRCP, TMAX, TMIN, del_TT_Deg_Celsius, DMI, Nino34,
	Air_200_Temp, Air_600_Temp, u_wind_200, u_wind_600, u_wind_850,
	v_wind_200, v_wind_600, v_wind_850) ~ year + month + LATITUDE + LONGITUDE,
	data=rain_mid, FUN=mean)
rain_hi_avg = aggregate(cbind(ELEVATION, PRCP, TMAX, TMIN, del_TT_Deg_Celsius, DMI, Nino34,
	Air_200_Temp, Air_600_Temp, u_wind_200, u_wind_600, u_wind_850,
	v_wind_200, v_wind_600, v_wind_850) ~ year + month + LATITUDE + LONGITUDE,
	data=rain_hi, FUN=mean)
...
### Depth PCA
yr.vec = 1976:2013
loadmat = matrix(0, ncol=16, nrow=length(yr.vec))

load.list.D = list(loadmat,loadmat,loadmat)
for(yr in yr.vec){
	pcmod.D = PcaRank(rain_lo_avg[,-c(1,2,6)], k=k)
	load.list.D[[1]][yr-1975,] = pcmod.D@loadings[,2]
	
	pcmod.D = PcaRank(rain_mid_avg[,-c(1,2,6)], k=k)
	load.list.D[[2]][yr-1975,] = pcmod.D@loadings[,2]

	pcmod.D = PcaRank(rain_hi_avg[,-c(1,2,6)], k=k)
	load.list.D[[3]][yr-1975,] = pcmod.D@loadings[,2]
}
...
for(yr in yr.vec){
	...
	pcmod = PcaClassic(rain_lo_avg[,-c(1,2,6)], k=k)
	load.list[[1]][yr-1975,] = pcmod@loadings[,1]
	
	pcmod = PcaClassic(rain_mid_avg[,-c(1,2,6)], k=k)
	load.list[[2]][yr-1975,] = pcmod@loadings[,1]

	pcmod = PcaClassic(rain_hi_avg[,-c(1,2,6)], k=k)
...
grid1 = unique(merge(datgrid1, grid, by=c("lon","lat"), all=T))

filled.contour(x=unique(grid1$lon), y=unique(grid1$lat), z=matrix(grid1$score, ncol=36, byrow=T),
